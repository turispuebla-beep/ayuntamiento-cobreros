...
const DESKTOP_PWA_HINT_DATE_KEY = 'desktopPwaHintFirstDate';
const DESKTOP_PWA_HINT_DAYS = 3; // Mostrar banner en PC durante 3 días
const MOBILE_PWA_HINT_DATE_KEY = 'mobilePwaHintFirstDate';
const MOBILE_PWA_HINT_DAYS = 5; // Mostrar banner en móvil durante 5 días
const DATOS_ENLACES_ALLOWED_FILE_TYPES = Object.freeze([
    '.pdf', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pptx',
    '.odt', '.ods', '.txt', '.jpg', '.jpeg', '.png', '.gif', '.webp', '.zip'
]);
const DATOS_ENLACES_ALLOWED_FILE_LABEL = 'PDF, Word, Excel, PowerPoint, imágenes (JPG/PNG/GIF/WebP), TXT, ZIP';
const DATOS_ENLACES_FILE_ACCEPT = DATOS_ENLACES_ALLOWED_FILE_TYPES.join(',');
...
const PWA_SUPPORT_ENABLED = true;

const PWA_INSTRUCTIONS = {
    android: `
        <div class="pwa-instructions-android">
            <p><strong>Instalar la app del Ayuntamiento en Android</strong></p>
            <ol>
                <li>Abre el menú <strong>⋮</strong> (tres puntos) en la esquina superior derecha de Chrome.</li>
                <li>Selecciona <strong>"Instalar app"</strong> o <strong>"Añadir a pantalla principal"</strong>.</li>
                <li>Confirma el nombre de la app y pulsa <strong>"Instalar"</strong> o <strong>"Añadir"</strong>.</li>
                <li>La app aparecerá en tu pantalla de inicio como una aplicación normal.</li>
        </ol>
            <p class="pwa-instructions-note">
                <strong>💡 Nota:</strong> Si no ves la opción "Instalar app", asegúrate de estar usando Chrome y de que la página esté completamente cargada.
            </p>
        </div>
    `,
    huawei: `
        <div class="pwa-instructions-huawei">
            <p><strong>Instalar la app del Ayuntamiento en Huawei/Honor</strong></p>
            <ol>
                <li>En el navegador (Chrome o navegador de Huawei), pulsa el menú <strong>⋮</strong> (tres puntos) en la esquina superior derecha.</li>
                <li>Selecciona <strong>"Agregar a pantalla principal"</strong> o <strong>"Instalar app"</strong>.</li>
                <li>Confirma con <strong>"Añadir"</strong> o <strong>"Instalar"</strong> para crear el acceso directo.</li>
                <li>La app aparecerá en tu pantalla de inicio y podrás usarla como una aplicación normal.</li>
        </ol>
            <p class="pwa-instructions-note">
                <strong>💡 Nota:</strong> Si usas AppGallery, también puedes instalar la app desde allí. La instalación desde el navegador funciona igual de bien.
            </p>
        </div>
    `,
    ios: `
        <div class="pwa-instructions-ios">
            <p><strong>Instalar la app del Ayuntamiento en iPhone o iPad</strong></p>
            <ol>
                <li>Abre esta página en <strong>Safari</strong> (no funciona en Chrome u otros navegadores).</li>
                <li>Pulsa el botón <strong>Compartir</strong> (<span aria-hidden="true">⬆️</span>) en la parte inferior de la pantalla (barra de herramientas).</li>
                <li>Desplázate hacia abajo y selecciona <strong>"Añadir a pantalla de inicio"</strong>.</li>
                <li>Personaliza el nombre si quieres (por defecto: "Ayuntamiento de Cobreros").</li>
                <li>Pulsa <strong>"Añadir"</strong> en la esquina superior derecha.</li>
                <li>La app aparecerá en tu pantalla de inicio con el icono del Ayuntamiento.</li>
        </ol>
            <p class="pwa-instructions-note">
                <strong>⚠️ Importante:</strong> Debes usar <strong>Safari</strong> para instalar la app. Chrome en iOS no permite instalar PWAs.
            </p>
            <p class="pwa-instructions-note">
                <strong>💡 Después de instalar:</strong> Abre la app desde la pantalla de inicio, completa tu registro y acepta las notificaciones para recibir avisos del Ayuntamiento.
            </p>
        </div>
    `
};
...
async function initializeApp() {
    // ✅ Migrado a Firebase Auth - Usar onAuthStateChanged para restaurar sesiones
    ...
    if (window.firebaseAuth && window.firebase?.onAuthStateChanged) {
        // Escuchar cambios en el estado de autenticación
        window.firebase.onAuthStateChanged(window.firebaseAuth, async (authUser) => {
            if (authUser) {
                ...
                try {
                    ...
                    let userData = null;
                    try {
                        const userDoc = await window.firebase.firestore().collection('users').doc(authUser.uid).get();
                        if (userDoc.exists) {
                    ...
                    const displayName = userData?.fullName || 
                                      (userData?.nombre ? `${userData.nombre}${userData.apellidos ? ' ' + userData.apellidos : ''}` : '') ||
                    ...
                    try {
                        const adminDoc = await window.firebase.firestore().collection('admins').doc(authUser.uid).get();
                        if (adminDoc.exists) {
                            ...
                            if (adminData?.isActive !== false) {
                                ...
                                if (adminData?.isSuperAdmin === true) {
                                    ...
                                    currentUser.team = adminData?.team || 'TURISTEAM';
                                }
                            }
                        }
                    } catch (error) {
        ...
        // Fallback: usar localStorage si Firebase Auth no está disponible aún
...
function createAdminButton() {
    ...
    adminBtn.addEventListener('click', () => openAdminLoginModalSafely('floating-button'));
...
function setupEventListeners() {
    ...
    if (adminLoginBtn) {
        adminLoginBtn.addEventListener('click', () => {
            ...
            openAdminLoginModalSafely('floating-button');
        });
    }
    ...
    if (adminBtn) {
        adminBtn.addEventListener('click', () => {
            ...
            if (!isAdmin || !currentUser) {
                ...
                openAdminLoginModalSafely('header-admin-btn');
                return;
            }
    ...
    const toggleAppointmentBtn = document.getElementById('toggleAppointmentForm');
    if (toggleAppointmentBtn) {
        toggleAppointmentBtn.addEventListener('click', toggleAppointmentForm);
    } else {
        Logger.warn('Botón para alternar formulario de cita previa no encontrado');
    }
    ...
    if (cancelAppointmentBtn) {
        ...
        Logger.warn('Botón para cancelar formulario de cita previa no encontrado');
    }
    ...
    if (adminLoginForm) {
        ...
        Logger.warn('Formulario de login de administrador no encontrado');
    }
    ...
    if (appointmentFormEl) {
        appointmentFormEl.addEventListener('submit', handleAppointment);
    } else {
        Logger.warn('Formulario de cita previa no encontrado');
    }
    ...
    if (logoForm) {
        ...
        Logger.warn('Formulario de logo no encontrado');
    }

    const createAdminFormEl = document.getElementById('createAdminForm');
    if (createAdminFormEl) {
        createAdminFormEl.addEventListener('submit', handleCreateAdmin);
    } else {
        Logger.warn('Formulario para crear administradores no encontrado');
    }
    ...
    if (documentUploadForm) {
        ...
        Logger.warn('Formulario de subida de documentos no encontrado');
    }

    const importDataForm = document.getElementById('importDataForm');
    if (importDataForm) {
        importDataForm.addEventListener('submit', handleDataImport);
    } else {
        Logger.warn('Formulario de importación de datos no encontrado');
    }
    ...
    if (mobileMenuToggle) {
        ...
        Logger.warn('Botón de menú móvil no encontrado');
    }
...
// Cargar eventos
// ✅ Migrado a Firestore - usar loadEventsFromFirestore()
function loadEvents() {
...
// ✅ Migrado a Firestore para persistencia completa
async function loadData() {
    console.log('📦 Cargando datos desde Firestore...');
    
    // Cargar todo en paralelo desde Firestore
    const loadPromises = [
        loadNewsFromFirestore(),
        loadBandosFromFirestore(),
        loadEventsFromFirestore(),
        loadDocumentsFromFirestore(),
        loadAdministratorsFromFirestore(),
        loadQuickAccessFromFirestore(),
        ...
        loadUsersFromFirestore().catch(() => {
            ...
            if (savedUsers) {
                try {
                    ...
                    console.log(`✅ ${users.length} usuarios cargados desde localStorage (fallback)`);
                } catch (error) {
...
async function loadNewsFromFirestore() {
    ...
    try {
        const snapshot = await window.firebase.firestore().collection('news').orderBy('date', 'desc').get();
        news = [];
        ...
        try {
            ...
            console.warn('No se pudo guardar noticias en localStorage:', storageError);
        }
        
        console.log(`✅ ${news.length} noticias cargadas desde Firestore`);
    } catch (error) {
        console.error('Error cargando noticias desde Firestore:', error);
        // Fallback a localStorage
        const savedNews = localStorage.getItem('news');
        if (savedNews) {
            news = JSON.parse(savedNews);
...
// ✅ Cargar bandos desde Firestore
async function loadBandosFromFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        // Fallback a localStorage
    ...
    try {
        const snapshot = await window.firebase.firestore().collection('bandos').orderBy('date', 'desc').get();
        bandos = [];
        snapshot.forEach(doc => {
            const data = doc.data();
            bandos.push({
        ...
        try {
            ...
            console.warn('No se pudo guardar bandos en localStorage:', storageError);
        }
        
        console.log(`✅ ${bandos.length} bandos cargados desde Firestore`);
        } catch (error) {
        console.error('Error cargando bandos desde Firestore:', error);
        // Fallback a localStorage
        const savedBandos = localStorage.getItem('bandos');
        if (savedBandos) {
            bandos = JSON.parse(savedBandos);
...
async function loadEventsFromFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        ...
        if (savedEvents) {
            events = JSON.parse(savedEvents).map(normalizeEventEntry);
    } else {
            events = [];
    ...
    try {
        const snapshot = await window.firebase.firestore().collection('events').orderBy('createdAt', 'desc').get();
        events = [];
        snapshot.forEach(doc => {
            const data = doc.data();
            // Convertir Firestore timestamps a ISO strings si es necesario
            let createdAt = data.createdAt;
            if (createdAt?.toDate) {
                createdAt = createdAt.toDate().toISOString();
            }
            events.push(normalizeEventEntry({
                id: doc.id,
                ...data,
        ...
        try {
            ...
            console.warn('No se pudo guardar eventos en localStorage:', storageError);
        }
...
async function loadDocumentsFromFirestore() {
    ...
    try {
        const snapshot = await window.firebase.firestore().collection('documents').orderBy('uploadedAt', 'desc').get();
        documents = [];
        snapshot.forEach(doc => {
            const data = doc.data();
            documents.push({
        ...
        try {
            ...
            console.warn('No se pudo guardar documentos en localStorage:', storageError);
        }
        
        console.log(`✅ ${documents.length} documentos cargados desde Firestore`);
    } catch (error) {
...
async function loadAdministratorsFromFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        // Fallback a localStorage
        const savedAdmins = localStorage.getItem('administrators');
        if (savedAdmins) {
    ...
    try {
        const snapshot = await window.firebase.firestore().collection('admins').get();
        administrators = [];
        snapshot.forEach(doc => {
            const data = doc.data();
        ...
        try {
            ...
            console.warn('No se pudo guardar administradores en localStorage:', storageError);
        }
        
        console.log(`✅ ${administrators.length} administradores cargados desde Firestore`);
    } catch (error) {
        console.error('Error cargando administradores desde Firestore:', error);
        // Fallback a localStorage
        const savedAdmins = localStorage.getItem('administrators');
        if (savedAdmins) {
            administrators = JSON.parse(savedAdmins);
...
// ✅ Cargar acceso rápido desde Firestore
async function loadQuickAccessFromFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        // Fallback a localStorage
        const savedQuickAccess = localStorage.getItem('quickAccess');
        if (savedQuickAccess) {
    ...
    try {
        const snapshot = await window.firebase.firestore().collection('quickAccess').orderBy('order').get();
        quickAccess = [];
        snapshot.forEach(doc => {
            const data = doc.data();
            quickAccess.push({
        ...
        // Si no hay acceso rápido en Firestore, usar valores por defecto
        ...
        try {
            ...
            console.warn('No se pudo guardar acceso rápido en localStorage:', storageError);
        }
        
        console.log(`✅ ${quickAccess.length} accesos rápidos cargados desde Firestore`);
    } catch (error) {
        console.error('Error cargando acceso rápido desde Firestore:', error);
        // Fallback a localStorage
        const savedQuickAccess = localStorage.getItem('quickAccess');
        if (savedQuickAccess) {
            quickAccess = JSON.parse(savedQuickAccess);
...
// ✅ Cargar notificaciones desde Firestore para persistencia
async function loadNotificationsFromFirestore() {
    ...
    try {
        // Cargar desde Firestore (últimas 100 notificaciones)
        ...
        try {
            ...
            console.warn('No se pudo guardar notificaciones en localStorage:', storageError);
        }

        console.log(`✅ ${notifications.length} notificaciones cargadas desde Firestore`);
    } catch (error) {
...
// ✅ Guardar noticias en Firestore para persistencia
async function saveNewsToFirestore() {
    ...
    try {
        // Obtener noticias actuales de Firestore para comparar
        const snapshot = await window.firebase.firestore().collection('news').get();
        const firestoreNews = {};
        snapshot.forEach(doc => {
            firestoreNews[doc.id] = doc.data();
        });

        // Guardar cada noticia en Firestore
        for (const newsItem of news) {
            const newsId = newsItem.id?.toString() || Date.now().toString();
            const newsData = { ...newsItem };
            delete newsData.id; // El ID será el documento en Firestore

            try {
                // Verificar si ya existe
                if (firestoreNews[newsId]) {
                    // Actualizar
                    await window.firebase.firestore().collection('news').doc(newsId).update(newsData);
                } else {
                    // Crear nuevo (usar ID como documento)
                    await window.firebase.firestore().collection('news').doc(newsId).set({
                        ...newsData,
                        createdAt: new Date().toISOString(),
                ...
                console.error(`Error guardando noticia ${newsId}:`, itemError);
            }
        }

        console.log(`✅ ${news.length} noticias guardadas en Firestore`);
    } catch (error) {
        console.error('Error guardando noticias en Firestore:', error);
        // No fallar si Firestore no está disponible
    }
}

// ✅ Guardar bandos en Firestore para persistencia
async function saveBandosToFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        return;
    }

    try {
        // Obtener bandos actuales de Firestore para comparar
        const snapshot = await window.firebase.firestore().collection('bandos').get();
        const firestoreBandos = {};
        snapshot.forEach(doc => {
            firestoreBandos[doc.id] = doc.data();
        });

        // Guardar cada bando en Firestore
        for (const bandoItem of bandos) {
            const bandoId = bandoItem.id?.toString() || Date.now().toString();
            const bandoData = { ...bandoItem };
            delete bandoData.id; // El ID será el documento en Firestore

            try {
                // Verificar si ya existe
                if (firestoreBandos[bandoId]) {
                    // Actualizar
                    await window.firebase.firestore().collection('bandos').doc(bandoId).update(bandoData);
                } else {
                    // Crear nuevo
                    await window.firebase.firestore().collection('bandos').doc(bandoId).set({
                        ...bandoData,
                        createdAt: new Date().toISOString(),
                        updatedAt: new Date().toISOString()
                    });
                ...
                console.error(`Error guardando bando ${bandoId}:`, itemError);
            }
        }

        console.log(`✅ ${bandos.length} bandos guardados en Firestore`);
    } catch (error) {
        console.error('Error guardando bandos en Firestore:', error);
        // No fallar si Firestore no está disponible
    }
}

// ✅ Guardar eventos en Firestore para persistencia
async function saveEventsToFirestore() {
    ...
    try {
        // Obtener eventos actuales de Firestore para comparar
        const snapshot = await window.firebase.firestore().collection('events').get();
        const firestoreEvents = {};
        snapshot.forEach(doc => {
            firestoreEvents[doc.id] = doc.data();
        });

        // Guardar cada evento en Firestore
        for (const eventItem of events) {
            const eventId = eventItem.id?.toString() || Date.now().toString();
            const eventData = { ...eventItem };
            delete eventData.id; // El ID será el documento en Firestore

            try {
                // Verificar si ya existe
                if (firestoreEvents[eventId]) {
                    // Actualizar
                    await window.firebase.firestore().collection('events').doc(eventId).update(eventData);
                } else {
                    // Crear nuevo
                    await window.firebase.firestore().collection('events').doc(eventId).set({
                        ...eventData,
                ...
                console.error(`Error guardando evento ${eventId}:`, itemError);
            }
        }

        console.log(`✅ ${events.length} eventos guardados en Firestore`);
    } catch (error) {
        console.error('Error guardando eventos en Firestore:', error);
        // No fallar si Firestore no está disponible
    }
}

// ✅ Guardar documentos en Firestore para persistencia
async function saveDocumentsToFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        return;
    }

    try {
        // Obtener documentos actuales de Firestore para comparar
        const snapshot = await window.firebase.firestore().collection('documents').get();
        const firestoreDocuments = {};
        snapshot.forEach(doc => {
            firestoreDocuments[doc.id] = doc.data();
        });

        // Guardar cada documento en Firestore
        for (const docItem of documents) {
            const docId = docItem.id?.toString() || Date.now().toString();
            const docData = { ...docItem };
            delete docData.id; // El ID será el documento en Firestore

            try {
                // Verificar si ya existe
                if (firestoreDocuments[docId]) {
                    // Actualizar
                    await window.firebase.firestore().collection('documents').doc(docId).update(docData);
                } else {
                    // Crear nuevo
                    await window.firebase.firestore().collection('documents').doc(docId).set({
                        ...docData,
                        createdAt: docData.uploadedAt || new Date().toISOString(),
                        updatedAt: new Date().toISOString()
                    });
                }
            } catch (itemError) {
                console.error(`Error guardando documento ${docId}:`, itemError);
            }
        }

        console.log(`✅ ${documents.length} documentos guardados en Firestore`);
    } catch (error) {
        console.error('Error guardando documentos en Firestore:', error);
        // No fallar si Firestore no está disponible
    }
}

// ✅ Guardar administradores en Firestore para persistencia (sin contraseñas)
async function saveAdministratorsToFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        return;
    }

    try {
        // Los administradores se gestionan desde Firebase Auth Console
        // Solo sincronizar información adicional si existe
        const snapshot = await window.firebase.firestore().collection('admins').get();
        const firestoreAdmins = {};
        snapshot.forEach(doc => {
            firestoreAdmins[doc.id] = doc.data();
        });

        // Sincronizar solo información adicional (sin contraseñas)
        for (const adminItem of administrators) {
            const adminId = adminItem.id?.toString() || (adminItem.email || Date.now().toString());
            const adminData = { ...adminItem };
            delete adminData.id;
            delete adminData.password; // NO guardar contraseñas en Firestore

            try {
                if (firestoreAdmins[adminId]) {
                    // Actualizar solo datos adicionales
                    await window.firebase.firestore().collection('admins').doc(adminId).update(adminData);
                } else {
                    // Crear nuevo (solo si no existe en Firestore)
                    await window.firebase.firestore().collection('admins').doc(adminId).set({
                        ...adminData,
                ...
                console.error(`Error guardando administrador ${adminId}:`, itemError);
            }
        }

        console.log(`✅ Administradores sincronizados con Firestore`);
    } catch (error) {
        console.error('Error guardando administradores en Firestore:', error);
        // No fallar si Firestore no está disponible
    }
}

// ✅ Guardar acceso rápido en Firestore para persistencia
async function saveQuickAccessToFirestore() {
    if (!window.firebase || !window.firebase.firestore) {
        return;
    }

    try {
        // Obtener acceso rápido actual de Firestore para comparar
        const snapshot = await window.firebase.firestore().collection('quickAccess').get();
        const firestoreQuickAccess = {};
        snapshot.forEach(doc => {
            firestoreQuickAccess[doc.id] = doc.data();
        });

        // Guardar cada acceso rápido en Firestore
        for (const quickItem of quickAccess) {
            const quickId = quickItem.id?.toString() || Date.now().toString();
            const quickData = { ...quickItem };
            delete quickData.id; // El ID será el documento en Firestore

            try {
                // Verificar si ya existe
                if (firestoreQuickAccess[quickId]) {
                    // Actualizar
                    await window.firebase.firestore().collection('quickAccess').doc(quickId).update(quickData);
                } else {
                    // Crear nuevo
                    await window.firebase.firestore().collection('quickAccess').doc(quickId).set({
                        ...quickData,
                ...
                console.error(`Error guardando acceso rápido ${quickId}:`, itemError);
            }
        }

        console.log(`✅ ${quickAccess.length} accesos rápidos guardados en Firestore`);
    } catch (error) {
        console.error('Error guardando acceso rápido en Firestore:', error);
        // No fallar si Firestore no está disponible
    }
}
...
function openAdminLoginModalSafely(source = 'unknown') {
    const logPrefix = `[AdminLoginModal][${source}]`;

    try {
        const modal = document.getElementById('adminLoginModal');
        if (!modal) {
            console.error(`${logPrefix} Modal adminLoginModal no encontrado en el DOM`);
            return false;
        }
        ...
        if (modalTitle && !modalTitle.id) {
            modalTitle.id = 'adminLoginModal-title';
        }
        if (modalTitle) {
        ...
        console.info(`${logPrefix} Modal abierto correctamente`);
        return true;
    } catch (error) {
        console.error(`${logPrefix} Error al abrir el modal`, error);
        if (typeof showFriendlyError === 'function') {
            showFriendlyError(error, 'abrir el acceso de administradores');
        }
        return false;
    }
}
...
if (typeof window !== 'undefined') {
    window.openAdminLoginModalSafely = openAdminLoginModalSafely;
}
...
function openModal(modalId) {
    ...
    if (modalId === 'adminModal') {
        if (!isAdmin || !currentUser) {
            ...
            openAdminLoginModalSafely('openModal-admin-redirect');
            return;
        }
    ...
    if (modalId === 'adminLoginModal') {
        openAdminLoginModalSafely('openModal-direct');
        return;
    }
...
// Manejar login de usuarios normales con Firebase Auth
async function handleLogin(e) {
    ...
    // Esperar a que Firebase Auth esté disponible
    if (!window.firebaseAuth || !window.firebase?.signInWithEmailAndPassword) {
        showNotification('El sistema de autenticación no está disponible. Por favor, recargue la página', 'error');
        return;
    }
...
// Manejar login de administradores con Firebase Auth
async function handleAdminLogin(eventOrEmail, maybePassword) {
    ...
    // Esperar a que Firebase Auth esté disponible
    if (!window.firebaseAuth || !window.firebase?.signInWithEmailAndPassword) {
        showAdminLoginError('El sistema de autenticación no está disponible. Por favor, recargue la página');
        return false;
    }
    ...
    try {
        ...
        if (!adminDoc.exists) {
            showAdminLoginError('No tiene permisos de administrador');
        ...
        if (adminData?.isActive === false) {
            showAdminLoginError('Su cuenta de administrador está desactivada');
            await window.firebase.signOut(window.firebaseAuth);
            return false;
        }
        ...
        currentUser = {
            uid: authUser.uid,
            email: authUser.email,
            name: adminData?.name || authUser.displayName || email.split('@')[0],
            isAdmin: true
        };
        ...
        showNotification(`Sesión de administrador iniciada - ${currentUser.name}`, 'success');
        return true;
...
async function handleCreateAdmin(e) {
    ...
    await saveAdministratorsToFirestore();
    // Fallback a localStorage
    try {
    localStorage.setItem('administrators', JSON.stringify(administrators));
    } catch (storageError) {
...
async function handleDocumentUpload(e) {
    ...
    // ✅ Guardar en Firestore para persistencia
    await saveDocumentsToFirestore();
    // Fallback a localStorage
    try {
    localStorage.setItem('documents', JSON.stringify(documents));
    } catch (storageError) {
...
async function handleAppointment(e) {
    ...
    // ✅ Migrado a Cloud Functions - Usar createAppointment
    // Obtener token de autenticación
    if (!window.firebaseAuth || !currentUser) {
        showNotification('Debe iniciar sesión para solicitar una cita previa', 'error');
        return;
    }
    ...
    try {
        ...
        const response = await fetch('https://us-central1-turisteam-80f1b.cloudfunctions.net/createAppointment', {
            method: 'POST',
            headers: {
        ...
        // Si hay adjunto, subirlo después de crear la cita
        if (attachmentFile && attachmentFile.size > 0 && result.appointmentId) {
            try {
                ...
                const attachmentResponse = await fetch('https://us-central1-turisteam-80f1b.cloudfunctions.net/uploadAppointmentAttachment', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${idToken}`
                ...
                if (!attachmentResponse.ok) {
                    console.warn('Error subiendo adjunto:', attachmentResult.error);
                    // No fallar la creación de la cita si falla el adjunto
                }
            } catch (attachmentError) {
                console.error('Error subiendo adjunto:', attachmentError);
                // No fallar la creación de la cita si falla el adjunto
            }
        }
        ...
        // Recargar citas desde Cloud Functions
        ...
        createMunicipalAlert({ ...appointmentData, id: result.appointmentId });
    ...
    if (users.length > 0) {
        ...
        showNotification(error.message || 'Hubo un problema al enviar la solicitud. Por favor, inténtelo de nuevo o contacte por teléfono.', 'error');
    }
}
...
async function sendNotificationToUsers(titleOrOptions, message, type, attachment = null, legacyOptions = {}) {
    ...
    // ✅ Guardar en Firestore para persistencia permanente
    if (window.firebase && window.firebase.firestore) {
        try {
            await window.firebase.firestore().collection('notifications').add({
                title: notification.title,
                message: notification.message,
                type: notification.type,
                date: notification.date,
                sent: notification.sent,
                attachment: notification.attachment || null,
                scope: scope || 'all',
                localities: scope === 'localities' ? localities : [],
                createdBy: currentUser?.uid || 'system',
                createdAt: new Date().toISOString()
            });
            console.log('✅ Notificación guardada en Firestore para persistencia');
        } catch (firestoreError) {
            console.error('Error guardando notificación en Firestore:', firestoreError);
            // No fallar si Firestore no está disponible, usar solo localStorage
        }
    }
...
// Verificar si debe mostrarse el banner en PC (3 días)
function shouldShowDesktopPwaHint() {
    try {
        // Si la app está instalada, no mostrar
        if (isPwaInstalled()) {
            return false;
        }
        
        const firstDateStr = localStorage.getItem(DESKTOP_PWA_HINT_DATE_KEY);
        if (!firstDateStr) {
            // Primera vez, guardar fecha de inicio
            localStorage.setItem(DESKTOP_PWA_HINT_DATE_KEY, getTodayDateKey());
            return true;
        }
        
        const firstDate = new Date(firstDateStr.replace(/(\d{4})-(\d{2})-(\d{2})/, '$1/$2/$3'));
        const today = new Date();
        const diffTime = today - firstDate;
        const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
        
        // Mostrar durante 3 días
        return diffDays < DESKTOP_PWA_HINT_DAYS;
    } catch (error) {
        Logger.warn?.('No se pudo consultar la fecha del aviso PWA de escritorio:', error);
        return true;
    }
}

// Verificar si debe mostrarse el banner en móvil (5 días)
function shouldShowMobilePwaHint() {
    try {
        // Si la app está instalada, no mostrar
        if (isPwaInstalled()) {
            return false;
        ...
        const firstDateStr = localStorage.getItem(MOBILE_PWA_HINT_DATE_KEY);
        if (!firstDateStr) {
            // Primera vez, guardar fecha de inicio
            localStorage.setItem(MOBILE_PWA_HINT_DATE_KEY, getTodayDateKey());
        return true;
        }
        ...
        // Mostrar durante 5 días
        return diffDays < MOBILE_PWA_HINT_DAYS;
    } catch (error) {
        Logger.warn?.('No se pudo consultar la fecha del aviso PWA de móvil:', error);
        return true;
    }
}

// Registrar que se mostró el banner (solo guarda fecha si no existe)
function registerDesktopPwaHintDisplay() {
    try {
        if (!localStorage.getItem(DESKTOP_PWA_HINT_DATE_KEY)) {
        localStorage.setItem(DESKTOP_PWA_HINT_DATE_KEY, getTodayDateKey());
        }
    } catch (error) {
        Logger.warn?.('No se pudo guardar la fecha del aviso PWA de escritorio:', error);
    }
}

// Registrar que se mostró el banner en móvil (solo guarda fecha si no existe)
function registerMobilePwaHintDisplay() {
    try {
        if (!localStorage.getItem(MOBILE_PWA_HINT_DATE_KEY)) {
            localStorage.setItem(MOBILE_PWA_HINT_DATE_KEY, getTodayDateKey());
        }
    } catch (error) {
        Logger.warn?.('No se pudo guardar la fecha del aviso PWA de móvil:', error);
    }
}
...
function isPwaInstalled() {
    // Verificar si está corriendo en modo standalone (app instalada)
    ...
    try {
        const installedFlag = localStorage.getItem(PWA_INSTALLED_KEY) === 'installed';
        
        // Si el flag dice que está instalada pero NO está en modo standalone,
        // significa que cambió de móvil o borró la app - limpiar flag
        if (installedFlag && !isRunningStandalone()) {
            // Verificar también si hay Service Worker activo
            if ('serviceWorker' in navigator) {
                navigator.serviceWorker.getRegistrations().then(registrations => {
                    const hasServiceWorker = Array.isArray(registrations) && registrations.length > 0;
                    if (!hasServiceWorker) {
                        // No hay Service Worker, la app ya no está instalada
                        clearPwaInstalledFlag();
                        // Resetear fechas de banner para volver a mostrar
                        localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
                        localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
                        console.log('🔄 App desinstalada detectada, reseteando banners');
                    }
                }).catch(() => {
                    // Si no puede verificar, asumir que no está instalada
                    clearPwaInstalledFlag();
                    localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
                    localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
                });
            } else {
                // No hay soporte de Service Worker, limpiar flag
                clearPwaInstalledFlag();
                localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
                localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
            }
            return false; // No está instalada realmente
        }
...
function renderPwaInstallBanner(force = false) {
    ...
    if (platform === 'desktop') {
        if (!force && !shouldShowDesktopPwaHint()) {
            return;
        }

        registerDesktopPwaHintDisplay();

        banner.innerHTML = `
            <div class="pwa-banner-content">
                <div class="pwa-banner-text">
                    <h3>📲 Recibe avisos del Ayuntamiento</h3>
                    <p><strong>En este PC:</strong> Regístrate e inicia sesión, luego acepta las notificaciones cuando tu navegador lo solicite. Recibirás los avisos directamente en tu escritorio.</p>
                    <p><strong>En tu móvil:</strong> Abre <strong>www.ayuntamientocobreros.com</strong> desde:</p>
                    <ul style="text-align: left; margin: 10px 0;">
                        <li><strong>Android/Huawei:</strong> Chrome o navegador del sistema</li>
                        <li><strong>iPhone/iPad:</strong> Safari (no funciona en Chrome)</li>
                    </ul>
                    <p>Instala la app móvil, regístrate y activa las notificaciones para recibir avisos en tu dispositivo móvil.</p>
                </div>
        ...
        // Verificar si debe mostrarse en móvil (5 días)
        if (!force && !shouldShowMobilePwaHint()) {
            return;
        }
        
        registerMobilePwaHintDisplay();
            ...
            <div class="pwa-banner-content">
                <div class="pwa-banner-header">
                    <div>
                        ...
                        <p>Disponible para Android, Huawei e iOS. Instálala, completa tu registro como usuario/a y acepta las notificaciones para recibir avisos directos en tu dispositivo móvil.</p>
                    </div>
...
async function validatePwaInstallationStatus(showWarnings = true) {
    ...
    if (!installationHealthy && installedFlag) {
        ...
        localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
        localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
        console.log('🔄 App desinstalada o perdida, reseteando banners');
        if (typeof Metrics !== 'undefined' && Metrics && typeof Metrics.recordEvent === 'function') {
    ...
    if (platform === 'android' || (platform === 'huawei' && actualPlatform === 'android')) {
        if (deferredPwaPrompt) {
            ...
            if (outcome !== 'accepted') {
                ...
                markPwaInstalled();
                // Limpiar fechas de banner ya que la app está instalada
                localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
                localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
                console.log('✅ App instalada desde prompt Android, banners desactivados');
                if (typeof Metrics !== 'undefined' && Metrics && typeof Metrics.recordEvent === 'function') {
    ...
    if (platform === 'huawei' && actualPlatform === 'huawei') {
        if (deferredPwaPrompt) {
            ...
            if (outcome === 'accepted') {
                ...
                localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
                localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
                console.log('✅ App instalada desde prompt Huawei, banners desactivados');
                if (typeof Metrics !== 'undefined' && Metrics && typeof Metrics.recordEvent === 'function') {
                    Metrics.recordEvent('pwa', 'install_prompt_accepted', {
                        platform: actualPlatform
                    });
...
function renderCarnetPublicInfo() {
    ...
    const feeProEl = document.getElementById('carnetFeeProfesional');
    ...
    if (feeProEl) {
        feeProEl.textContent = (carnetConfig.feeProfesional || 'Consultar');
    }
...
function populateCarnetAdminForm() {
    ...
    const feeProInput = document.getElementById('carnetConfigFeeProfesional');
    if (feeProInput) {
        feeProInput.value = carnetConfig.feeProfesional || '';
    }
    ...
    try {
        ...
        if (docRef && typeof docRef.set === 'function') {
            await docRef.set({
                ...
                feeProfesional: carnetConfig.feeProfesional || '',
...
async function handleCarnetConfigSave() {
    ...
    const updates = {
        ...
        feeProfesional: (document.getElementById('carnetConfigFeeProfesional')?.value || '').trim(),
...
function resetCarnetForm(form) {
    ...
    updateCarnetTipoResumen(null);
}

function setupCarnetTipoSummary() {
    ...
    radios.forEach(radio => {
        radio.addEventListener('change', () => updateCarnetTipoResumen(radio.value));
    });
    const checked = form.querySelector('input[name="tipoSolicitante"]:checked');
    updateCarnetTipoResumen(checked ? checked.value : null);
}

function updateCarnetTipoResumen(selectedType) {
    const summary = document.getElementById('carnetTipoResumen');
    if (!summary) return;
    summary.querySelectorAll('[data-type]').forEach(item => {
        const isActive = item.dataset.type === selectedType && selectedType !== null;
...
function getCarnetFeeByType(type) {
    ...
    if (type === 'profesional') {
        return carnetConfig.feeProfesional || '';
    }
    return '';
}
...
function normalizeEventEntry(event) {
    if (!event) return { showOnHome: false };
    return {
        ...event,
        ...
        eventItem.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: start;">
                <div style="flex: 1;">
                    ...
                    <p><strong>Visible en la página principal:</strong> ${event.showOnHome ? 'Sí' : 'No'}</p>
                </div>
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
...
// ✅ Cargar historial de notificaciones (desde Firestore si está disponible)
async function loadNotificationsHistory() {
    ...
    // Si hay notificaciones en memoria, usarlas; si no, cargar desde Firestore
...
async function deleteDocument(docId) {
    ...
    // ✅ Eliminar de Firestore para persistencia
    if (window.firebase && window.firebase.firestore) {
        try {
            const docIdStr = docId?.toString() || doc.id?.toString();
            await window.firebase.firestore().collection('documents').doc(docIdStr).delete();
            console.log(`✅ Documento ${docIdStr} eliminado de Firestore`);
        } catch (error) {
    ...
    localStorage.setItem('documents', JSON.stringify(documents));
        ...
        console.warn('No se pudo actualizar localStorage:', storageError);
    }
...
async function deleteNews(newsId) {
    ...
    if (window.firebase && window.firebase.firestore) {
        try {
            const newsIdStr = newsId?.toString() || newsItem.id?.toString();
            await window.firebase.firestore().collection('news').doc(newsIdStr).delete();
            console.log(`✅ Noticia ${newsIdStr} eliminada de Firestore`);
        } catch (error) {
            console.error('Error eliminando noticia de Firestore:', error);
            // Continuar aunque falle Firestore
        }
    }
...
async function deleteBando(bandoId) {
    ...
    if (window.firebase && window.firebase.firestore) {
        try {
            const bandoIdStr = bandoId?.toString() || bandoItem.id?.toString();
            await window.firebase.firestore().collection('bandos').doc(bandoIdStr).delete();
            console.log(`✅ Bando ${bandoIdStr} eliminado de Firestore`);
        } catch (error) {
            console.error('Error eliminando bando de Firestore:', error);
            // Continuar aunque falle Firestore
        }
    }
...
function openEventEditor(eventId = null) {
    ...
    const visibleCheckbox = document.getElementById('eventVisibleHome');
    ...
    if (eventId) {
        ...
        if (event) {
            ...
            if (visibleCheckbox) {
                visibleCheckbox.checked = event.showOnHome === true;
            }
...
async function saveEvent() {
    ...
    const eventData = {
        ...
        showOnHome: visibleCheckbox ? visibleCheckbox.checked : false
    };
...
async function deleteEvent(eventId) {
    if (confirm('¿Estás seguro de que quieres eliminar este evento?')) {
        ...
        if (window.firebase && window.firebase.firestore) {
            try {
                const eventIdStr = eventId?.toString() || eventItem?.id?.toString();
                if (eventIdStr) {
                    await window.firebase.firestore().collection('events').doc(eventIdStr).delete();
                    console.log(`✅ Evento ${eventIdStr} eliminado de Firestore`);
                }
            } catch (error) {
...
function renderEventosDestacados() {
    const section = document.getElementById('eventosDestacados');
    const grid = document.getElementById('eventosDestacadosGrid');
    if (!section || !grid) return;

    const destacados = events
    ...
    grid.innerHTML = destacados.map(event => {
        const eventDate = event.date ? new Date(event.date) : null;
        const day = eventDate && !isNaN(eventDate) ? eventDate.toLocaleDateString('es-ES', { day: '2-digit' }) : '--';
        const month = eventDate && !isNaN(eventDate) ? eventDate.toLocaleDateString('es-ES', { month: 'short' }).toUpperCase() : '';
        const time = event.time || 'Horario por confirmar';
        const location = event.location || 'Lugar por confirmar';
            ...
            <article class="evento-card">
                ...
                <div class="evento-card-content">
                    <div class="evento-card-header">
                        <span class="evento-chip">${category}</span>
                        ${event.image ? `<span class="evento-chip chip-photo">📷</span>` : ''}
                    </div>
                    <h3>${event.title}</h3>
...
// Funciones para gestión de citas previas - ✅ Migrado a Cloud Functions
async function loadAppointments() {
    // Verificar si hay usuario autenticado
    if (!window.firebaseAuth || !currentUser) {
        console.log('⚠️ No hay usuario autenticado, no se pueden cargar citas');
        appointments = [];
        return;
    }
    ...
    try {
        ...
        // Cargar citas desde Cloud Function
        const response = await fetch('https://us-central1-turisteam-80f1b.cloudfunctions.net/getAppointments?limit=1000', {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${idToken}`
            }
        ...
        if (result.success && result.appointments) {
            appointments = result.appointments.map(apt => {
                ...
                if (apt.createdAt?.toDate) {
                    apt.createdAt = apt.createdAt.toDate().toISOString();
                }
                if (apt.updatedAt?.toDate) {
                    apt.updatedAt = apt.updatedAt.toDate().toISOString();
                }
            ...
            console.log(`✅ ${appointments.length} citas cargadas desde Cloud Functions`);
        } else {
            appointments = [];
            console.log('⚠️ No hay citas disponibles');
        }
        } catch (error) {
        console.error('❌ Error cargando citas desde Cloud Functions:', error);
        // Fallback a localStorage si hay datos antiguos
        const savedAppointments = localStorage.getItem('appointments');
        if (savedAppointments) {
            try {
                appointments = JSON.parse(savedAppointments);
                console.log(`⚠️ Cargadas ${appointments.length} citas desde localStorage (fallback)`);
            } catch (parseError) {
                console.error('❌ Error parseando citas de localStorage:', parseError);
            appointments = [];
        }
    } else {
        appointments = [];
...
// ✅ Migrado a Cloud Functions - ya no se guarda en localStorage
// Esta función se mantiene para compatibilidad pero no hace nada
// Las citas se guardan automáticamente en Firestore a través de Cloud Functions
function saveAppointments() {
    // No hacer nada - las citas se guardan en Firestore vía Cloud Functions
    // Mantener para compatibilidad con código existente que llama a esta función
    console.log('💡 saveAppointments: Las citas se guardan automáticamente en Firestore vía Cloud Functions');
}
...
// Función de login (legacy - usa handleLogin)
async function login() {
    ...
    // Usar handleLogin que ya usa Firebase Auth
...
async function logout() {
    try {
        ...
        if (window.firebaseAuth && window.firebase?.signOut) {
            await window.firebase.signOut(window.firebaseAuth);
        }
    } catch (error) {
...
function openAdminPanel() {
    ...
    if (!isAdmin || !currentUser) {
        ...
        openAdminLoginModalSafely('open-admin-panel-no-session');
        return;
    }
    ...
    if (!savedUser || savedAdmin !== 'true') {
        ...
        localStorage.removeItem('isAdmin');
        updateUserInterface();
        openAdminLoginModalSafely('open-admin-panel-invalid-session');
        return;
    }
...
async function uploadServicioAsset(file, { type, kind, entityId }) {
    ...
    const extensionWhitelist = kind === 'document'
        ? DATOS_ENLACES_ALLOWED_FILE_TYPES
        : ['.jpg', '.jpeg', '.png', '.webp'];
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <form id="servicioForm">
                ...
                <div class="form-group">
                    <label for="servicioDocumentFile">Documento (${DATOS_ENLACES_ALLOWED_FILE_LABEL})</label>
                    <input type="file" id="servicioDocumentFile" name="servicioDocumentFile" accept="${DATOS_ENLACES_FILE_ACCEPT}">
                    <small style="color: #6b7280;">Sube archivos (${DATOS_ENLACES_ALLOWED_FILE_LABEL}) o indica una URL pública.</small>
        ...
        <div class="modal-content" style="max-width: 600px;">
            ...
            <div class="modal-body">
                <form id="transporteDocumentoForm">
                    ...
                    <div class="form-group">
                        <label for="documentoFile">Subir archivo (${DATOS_ENLACES_ALLOWED_FILE_LABEL})</label>
                        <input type="file" id="documentoFile" name="documentoFile" accept="${DATOS_ENLACES_FILE_ACCEPT}">
                        <small style="color: #6b7280;">Si subes un archivo nuevo se almacenará en Firebase Storage (${DATOS_ENLACES_ALLOWED_FILE_LABEL}).</small>
...
async function deleteAdmin(adminId) {
    ...
    const adminIdStr = adminId?.toString() || admin.id?.toString();
    ...
    if (window.firebase && window.firebase.firestore) {
        try {
            await window.firebase.firestore().collection('admins').doc(adminIdStr).delete();
            console.log(`✅ Administrador ${adminIdStr} eliminado de Firestore`);
        } catch (error) {
            console.error('Error eliminando administrador de Firestore:', error);
            // Continuar aunque falle Firestore
        }
    }
...
function registerServiceWorker() {
    if (!PWA_SUPPORT_ENABLED) {
        console.log('ℹ️ Service Worker deshabilitado temporalmente para despliegue');
        return;
    }
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
...
function showPWAInstallBanner() {
    ...
    const platform = detectDevicePlatform();
    // Mostrar banner en todas las plataformas (móvil y desktop)
...
if (typeof window !== 'undefined') {
    window.addEventListener('appinstalled', () => {
        ...
        localStorage.removeItem(DESKTOP_PWA_HINT_DATE_KEY);
        localStorage.removeItem(MOBILE_PWA_HINT_DATE_KEY);
        console.log('✅ App instalada, banners desactivados');
        if (typeof Metrics !== 'undefined' && Metrics && typeof Metrics.recordEvent === 'function') {
...
function initializePWA() {
    if (!PWA_SUPPORT_ENABLED) {
        console.log('ℹ️ PWA deshabilitada temporalmente, omitiendo inicialización');
        return;
    }
...
// Mostrar notificación web (funciona en PC y móvil)
function showWebNotification(notificationData) {
    ...
    try {
        const notification = new Notification(notificationData.title || '🏛️ Ayuntamiento de Cobreros', options);
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <form id="consultorioDocumentForm" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="docFile">Subir archivo (${DATOS_ENLACES_ALLOWED_FILE_LABEL}):</label>
                    <input type="file" id="docFile" accept="${DATOS_ENLACES_FILE_ACCEPT}" onchange="handleFileUpload('docFile', 'docUrl')">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <small>Puedes subir un archivo (${DATOS_ENLACES_ALLOWED_FILE_LABEL}) o proporcionar una URL pública. Si subes un archivo, se usará automáticamente.</small>
                </div>
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <form id="itvDocumentForm" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="itvDocFile">Subir archivo (${DATOS_ENLACES_ALLOWED_FILE_LABEL}):</label>
                    <input type="file" id="itvDocFile" accept="${DATOS_ENLACES_FILE_ACCEPT}" onchange="handleFileUpload('itvDocFile', 'itvDocUrl')">
                </div>
                <div class="form-group">
    ...
    if (!savedData || Object.values(culturaOcioData).every(section => section.length === 0)) {
        const cobrerosData = {
            naturaleza: [
                ...
                {
                    id: generateId(),
                    title: "💧 Cascada de Aguas Cernidas",
                    description: "Salto de agua escondido entre robledales y brezales, con pozas naturales perfectas para descansar tras la caminata. Ruta circular de baja dificultad.",
                    image: "images/cascada-aguas-cernidas.jpg",
                    links: [
                        { text: "📋 Ficha de la Ruta", url: "#", type: "pdf" },
                ...
                {
                    id: generateId(),
                    title: "🌄 Laguna de Sotillo",
                    description: "Laguna de origen glaciar a 1.600 metros de altitud, rodeada de pinos silvestres y brezales. Mirador espectacular del valle de Sanabria.",
                    image: "images/laguna-sotillo.jpg",
                    links: [
                        { text: "🥾 Ruta completa", url: "#", type: "pdf" },
                        { text: "📌 Puntos de interés", url: "#", type: "external" }
                    ],
                    order: 4
                }
            ],
