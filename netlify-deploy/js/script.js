...
const DESKTOP_PWA_HINT_COUNT_KEY = 'desktopPwaHintCount';
const DESKTOP_PWA_HINT_DATE_KEY = 'desktopPwaHintLastDate';
const DESKTOP_PWA_HINT_MAX = 3;
...
// Exponer credenciales globalmente INMEDIATAMENTE para que otros scripts puedan acceder
// incluso si script.js falla más adelante
if (typeof window !== 'undefined') {
    window.SUPER_ADMIN = SUPER_ADMIN;
    window.ADMIN_CREDENTIALS = ADMIN_CREDENTIALS;
    // También exponer administrators si está disponible
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof TransparencyPortal !== 'undefined' && TransparencyPortal.init) {
        setTimeout(() => TransparencyPortal.init(), 1000);
    }
    if (typeof ExpedientQuery !== 'undefined' && ExpedientQuery.init) {
        setTimeout(() => ExpedientQuery.init(), 1000);
    }
    if (typeof CitizenParticipation !== 'undefined' && CitizenParticipation.init) {
        setTimeout(() => CitizenParticipation.init(), 1000);
    }
    if (typeof OnlinePayments !== 'undefined' && OnlinePayments.init) {
        setTimeout(() => OnlinePayments.init(), 1000);
    }
...
function setupEventListeners() {
    ...
    if (loginBtn) {
        ...
        // Fallback adicional: asegurar que el botón tenga onclick como respaldo
        loginBtn.setAttribute('data-modal', 'loginModal');
    ...
    if (registerBtn) {
        ...
        // Fallback adicional: asegurar que el botón tenga onclick como respaldo
        registerBtn.setAttribute('data-modal', 'registerModal');
    ...
    document.querySelectorAll('.close').forEach(closeBtn => {
        ...
        // Fallback adicional: onclick directo
    ...
    const adminLoginForm = document.getElementById('adminLoginForm');
    if (adminLoginForm) {
        // Esperar un poco para ver si recaptcha.js se carga y maneja el formulario
        // Si después de 1 segundo recaptcha.js no se ha inicializado, agregar nuestro listener
        setTimeout(() => {
            if (typeof window.recaptchaInitialized === 'undefined' || !window.recaptchaInitialized) {
                // recaptcha.js no está disponible, usar listener directo
                adminLoginForm.addEventListener('submit', handleAdminLogin);
                Logger.log('Listener de admin login agregado (recaptcha.js no disponible)');
            } else {
                Logger.log('recaptcha.js manejará el formulario de admin login');
            }
        }, 1000);
    } else {
...
function updateActiveNavLink(activeLink) {
    ...
    if (activeLink) {
        ...
        // Inicializar módulos cuando se navega a sus secciones
        const href = activeLink.getAttribute('href');
        if (href === '#transparencia' && typeof TransparencyPortal !== 'undefined' && TransparencyPortal.init) {
            setTimeout(() => TransparencyPortal.init(), 300);
        }
        if (href === '#expedientes' && typeof ExpedientQuery !== 'undefined' && ExpedientQuery.init) {
            setTimeout(() => ExpedientQuery.init(), 300);
        }
        if (href === '#participacion' && typeof CitizenParticipation !== 'undefined' && CitizenParticipation.init) {
            setTimeout(() => CitizenParticipation.init(), 300);
        }
        if (href === '#pagos-online' && typeof OnlinePayments !== 'undefined' && OnlinePayments.init) {
            setTimeout(() => OnlinePayments.init(), 300);
        }
    }
}
...
function openModal(modalId) {
    try {
        ...
        console.log('Modal found:', !!modal, 'Modal ID:', modalId);
        ...
        if (typeof showNotification === 'function') {
            showNotification('Error al abrir el modal. Por favor, recarga la página.', 'error');
        }
    }
}

// Exponer openModal globalmente
if (typeof window !== 'undefined') {
...
async function handleAdminLogin(eventOrEmail, maybePassword, recaptchaToken) {
    ...
    if (typeof window !== 'undefined' && window.inputSanitizer) {
        ...
        password = window.inputSanitizer.sanitizeText(password);
    }

    // Rate limiting mejorado - Verificar bloqueo antes de intentar login
    if (typeof window !== 'undefined' && window.enhancedRateLimiter) {
        const blocked = await window.enhancedRateLimiter.checkIfBlocked(email);
        if (blocked.blocked) {
            showAdminLoginError(`Acceso bloqueado. Intente de nuevo en ${blocked.remainingMinutes} minutos.`);
            return false;
        }
    }

    // Debug: Log para verificar qué está pasando
    console.log('🔍 [DEBUG] Intentando login admin:', {
        emailProvided: email,
        passwordLength: password ? password.length : 0,
        superAdminEmail: SUPER_ADMIN?.email,
        adminEmail: ADMIN_CREDENTIALS?.email,
        hasSuperAdmin: !!SUPER_ADMIN,
        hasAdminCredentials: !!ADMIN_CREDENTIALS
    });

    if (email === SUPER_ADMIN.email && password === SUPER_ADMIN.password) {
        console.log('✅ [DEBUG] Login como SUPER_ADMIN exitoso');
        
        // Limpiar intentos fallidos
        if (typeof window !== 'undefined' && window.enhancedRateLimiter) {
            await window.enhancedRateLimiter.clearFailedAttempts(email);
        }
        
        // Verificar verificación de email (opcional para super admin)
        if (typeof window !== 'undefined' && window.emailVerification) {
            const requiresVerification = await window.emailVerification.requireVerificationBeforeAccess('super_admin');
            if (requiresVerification) {
                // Super admin puede saltarse verificación, pero registrar
                console.log('⚠️ Super admin sin verificación de email');
            }
        }
        ...
        if (typeof window !== 'undefined' && window.sessionManager) {
            // Opción de recordar sesión (se puede agregar checkbox en el formulario)
            const rememberSession = form?.querySelector('#rememberAdminSession')?.checked || false;
            if (rememberSession) {
                window.sessionManager.saveRememberedSession(currentUser, 30);
            }
        }

        // Logs de auditoría mejorados
        if (typeof window !== 'undefined' && window.auditLogSystem) {
            await window.auditLogSystem.log('ADMIN_LOGIN', {
                adminType: 'super_admin',
                adminName: SUPER_ADMIN.name,
                ipAddress: await getClientIP() || 'unknown'
            }, currentUser.email, currentUser.email);
        } else if (typeof logAuditAction === 'function') {
        ...
        if (typeof window !== 'undefined' && window.sessionManager) {
            await window.sessionManager.notifyNewLogin();
        }

        // Detectar login desde nueva ubicación/IP
        await detectAndNotifyNewLocationLogin(currentUser.email, currentUser.name || SUPER_ADMIN.name);

        updateUserInterface();
        finalizeSuccessLogin();
        showNotification('✅ Sesión de administrador iniciada correctamente. Bienvenido al Panel de Administración.', 'success');
        return true;
    }
    ...
    console.log('🔍 [DEBUG] Verificando credenciales de admin:', {
        emailMatch: email === adminEmail,
        passwordMatch: password === adminPassword
    });

    if (email === adminEmail && password === adminPassword) {
        console.log('✅ [DEBUG] Login como ADMIN exitoso');
        ...
        if (typeof window !== 'undefined' && window.emailVerification) {
            const requiresVerification = await window.emailVerification.requireVerificationBeforeAccess('default_admin');
            if (requiresVerification) {
                showAdminLoginError('Debe verificar su email antes de acceder. Revise su bandeja de entrada.');
                return false;
            }
        }
        ...
        // Detectar login desde nueva ubicación/IP
        await detectAndNotifyNewLocationLogin(currentUser.email, ADMIN_CREDENTIALS.name);

        updateUserInterface();
        finalizeSuccessLogin();
        showNotification('✅ Sesión de administrador iniciada correctamente. Bienvenido, Super Administrador.', 'success');
        return true;
    }

    console.log('🔍 [DEBUG] Buscando en lista de administradores:', {
        totalAdmins: administrators?.length || 0,
        emailToFind: email
    });
    ...
    if (admin) {
        console.log('✅ [DEBUG] Login como administrador personalizado exitoso:', admin.name);
        ...
        // Verificar verificación de email
        if (typeof window !== 'undefined' && window.emailVerification) {
            const requiresVerification = await window.emailVerification.requireVerificationBeforeAccess(admin.id);
            if (requiresVerification) {
                showAdminLoginError('Debe verificar su email antes de acceder. Revise su bandeja de entrada.');
                return false;
            }
        ...
        updateUserInterface();
        finalizeSuccessLogin();
        showNotification(`✅ Sesión de administrador iniciada correctamente. Bienvenido/a, ${admin.name}.`, 'success');
        return true;
    }

    console.log('❌ [DEBUG] Login fallido - credenciales incorrectas');
    
    // Registrar intento fallido en rate limiter
    if (typeof window !== 'undefined' && window.enhancedRateLimiter) {
        const result = await window.enhancedRateLimiter.recordFailedAttempt(email);
        if (!result.allowed) {
            showAdminLoginError(result.message);
        } else if (result.attemptsRemaining < 3) {
            showAdminLoginError(`Credenciales incorrectas. ${result.message}`);
        } else {
    ...
    if (typeof window !== 'undefined' && window.auditLogSystem) {
        await window.auditLogSystem.log('ADMIN_LOGIN_FAILED', {
            email: email,
            reason: 'invalid_credentials'
        });
    }
...
// Exponer handleAdminLogin globalmente INMEDIATAMENTE después de su definición
// para que otros scripts (como recaptcha.js) puedan acceder a ella
// Esto sobrescribe el fallback si existe
if (typeof window !== 'undefined') {
    window.handleAdminLogin = handleAdminLogin;
    // Marcar que script.js se ha cargado correctamente
    window.scriptJsLoaded = true;
    if (typeof console !== 'undefined' && console.log) {
        console.log('✅ handleAdminLogin expuesta globalmente (versión completa de script.js)');
    }
}
...
async function handleRegister(e, recaptchaToken = null) {
    ...
    const userName = `${userData.nombre} ${userData.surname1 || ''}`.trim();
    showNotification(`✅ Registro completado correctamente. Bienvenido/a, ${userName}. Ahora recibirá notificaciones y avisos del ayuntamiento.`, 'success');
        ...
        showNotification('❌ Error al registrar. Por favor, verifique su conexión e inténtelo de nuevo.', 'error');
        ...
        if (form) {
            ...
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = btn.innerHTML.replace(/<i class="fas fa-spinner fa-spin"><\/i> Registrando\.\.\./, 'Registrarse');
            }
        }
    }
}
...
async function handleCreateAdmin(e) {
    ...
    if (typeof window !== 'undefined' && window.auditLogSystem) {
        await window.auditLogSystem.log('ADMIN_CREATED', {
            ...
            ipAddress: await getClientIP() || 'unknown'
        }, currentUser?.email || 'system', currentUser?.email || 'system');
    }

    // Enviar notificación de seguridad por email
    try {
        await sendSecurityNotification('admin_created', {
            adminEmail: email,
            adminName: name,
            createdBy: currentUser?.name || currentUser?.email || 'Sistema',
            createdByEmail: currentUser?.email || 'system',
            timestamp: new Date().toLocaleString('es-ES')
        });
    } catch (error) {
        console.warn('No se pudo enviar notificación de seguridad:', error);
    }
...
async function handleAppointment(e) {
    ...
    if (confirmationSent && alertSent) {
        ...
        const serviceName = getServiceName(appointmentData.service);
        const dateFormatted = formatDateForDisplay(appointmentData.date);
        showNotification(`✅ Solicitud de cita previa enviada correctamente. Servicio: ${serviceName}, Fecha: ${dateFormatted} a las ${appointmentData.time}. Recibirá un email de confirmación en breve.`, 'success');
        ...
        showNotification('❌ Hubo un problema al enviar la solicitud. Por favor, verifique su conexión e inténtelo de nuevo. Si el problema persiste, contacte por teléfono.', 'error');
        // Restaurar botón si existe
        ...
        if (form) {
            ...
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = btn.innerHTML.replace(/<i class="fas fa-spinner fa-spin"><\/i> Procesando solicitud\.\.\./, 'Solicitar Cita');
            }
        }
        return;
    }

    // Enviar notificación push a usuarios registrados (si está habilitado)
    const appointmentSettings = JSON.parse(localStorage.getItem('appointmentSettings') || '{}');
    const sendPushOnAppointment = appointmentSettings.sendPushNotification === true;
    
    if (users.length > 0 && sendPushOnAppointment) {
        await sendNotificationToUsers({
            title: 'Nueva solicitud de cita previa',
            message: `Se ha recibido una nueva solicitud de cita para ${getServiceName(appointmentData.service)} de ${appointmentData.name}`,
            type: 'cita',
            sendPush: true,
            sendEmail: false
        });
...
function switchTab(tabName) {
    ...
    if (tabName === 'content') {
        ...
        loadSystemStats();
    } else if (tabName === 'transparency-admin') {
        // Cargar listas de transparencia
        if (typeof loadTransparencyAdminList === 'function') {
            setTimeout(() => {
                loadTransparencyAdminList('budget');
                loadTransparencyAdminList('contract');
                loadTransparencyAdminList('plenary');
            }, 300);
        }
    } else if (tabName === 'expedients-admin') {
        // Cargar expedientes
        if (typeof loadAllExpedients === 'function') {
            setTimeout(() => loadAllExpedients(), 300);
        }
    } else if (tabName === 'participation-admin') {
        // Cargar encuestas y sugerencias
        if (typeof loadAllSurveys === 'function' && typeof loadAllSuggestions === 'function') {
            setTimeout(() => {
                loadAllSurveys();
                loadAllSuggestions();
            }, 300);
        ...
        // Agregar botón de configuración del sistema si no existe
        setTimeout(() => {
            const settingsTab = document.getElementById('settings-tab');
            if (settingsTab && !document.getElementById('systemConfigButton')) {
                const systemConfigSection = document.createElement('div');
                systemConfigSection.className = 'admin-section';
                systemConfigSection.style.marginTop = '2rem';
                systemConfigSection.innerHTML = `
                    <h4>⚙️ Configuración del Sistema</h4>
                    <p>Gestiona la configuración de todos los módulos del sistema.</p>
                    <button id="systemConfigButton" class="btn btn-primary" onclick="window.systemConfigPanel?.show()">
                        <i class="fas fa-cog"></i> Abrir Panel de Configuración
                    </button>
                `;
                settingsTab.appendChild(systemConfigSection);
            }
        }, 200);
        ...
        if (typeof loadParticipationSettings === 'function') {
            setTimeout(() => loadParticipationSettings(), 100);
        }
        // Cargar encuestas y sugerencias
        if (typeof loadAllSurveys === 'function' && typeof loadAllSuggestions === 'function') {
            setTimeout(() => {
    ...
    } else if (tabName === 'payments-admin') {
        // Cargar configuración de pagos
        if (typeof loadPaymentsConfig === 'function') {
            setTimeout(() => loadPaymentsConfig(), 100);
        }
        // Cargar servicios pagables
        if (typeof loadAllPayableServices === 'function') {
            setTimeout(() => loadAllPayableServices(), 300);
        }
...
function shouldShowDesktopPwaHint() {
    try {
        const count = parseInt(localStorage.getItem(DESKTOP_PWA_HINT_COUNT_KEY) || '0', 10);
        const lastDate = localStorage.getItem(DESKTOP_PWA_HINT_DATE_KEY);
        const today = getTodayDateKey();

        if (Number.isNaN(count)) {
            return true;
        }
        if (count >= DESKTOP_PWA_HINT_MAX) {
            return false;
        }
        if (lastDate === today) {
            return false;
        ...
        Logger.warn?.('No se pudo consultar el contador del aviso PWA de escritorio:', error);
        return true;
    }
}
...
function registerDesktopPwaHintDisplay() {
    try {
        const currentCount = parseInt(localStorage.getItem(DESKTOP_PWA_HINT_COUNT_KEY) || '0', 10);
        const nextCount = Number.isNaN(currentCount) ? 1 : Math.min(DESKTOP_PWA_HINT_MAX, currentCount + 1);
        localStorage.setItem(DESKTOP_PWA_HINT_COUNT_KEY, String(nextCount));
        localStorage.setItem(DESKTOP_PWA_HINT_DATE_KEY, getTodayDateKey());
    } catch (error) {
        Logger.warn?.('No se pudo actualizar el contador del aviso PWA de escritorio:', error);
    }
}
...
function loadAppointmentSettings() {
    ...
    if (savedSettings) {
        ...
        appointmentsEnabled = settings.enabled;
            ...
            // Cargar estado del checkbox de push notification
            const sendPushCheckbox = document.getElementById('appointmentSendPushNotification');
            if (sendPushCheckbox && settings.sendPushNotification !== undefined) {
                sendPushCheckbox.checked = settings.sendPushNotification === true;
            }
        } catch (error) {
    ...
    // Actualizar checkbox de push notification
    if (sendPushCheckbox) {
        const savedSettings = localStorage.getItem('appointmentSettings');
        if (savedSettings) {
            try {
                const settings = JSON.parse(savedSettings);
                if (settings.sendPushNotification !== undefined) {
                    sendPushCheckbox.checked = settings.sendPushNotification === true;
                    console.log('🔔 Checkbox de push notification actualizado:', sendPushCheckbox.checked ? 'Habilitado' : 'Deshabilitado');
                }
            } catch (error) {
                console.error('❌ Error cargando configuración de push notification:', error);
            }
        }
    }
...
function updateAppointmentMode() {
    ...
    const sendPushOnAppointment = sendPushCheckbox ? sendPushCheckbox.checked : false;
    
    // Guardar configuración con múltiple seguridad
    const settings = {
        enabled: appointmentsEnabled,
        sendPushNotification: sendPushOnAppointment,
...
// Función para detectar y notificar logins desde nuevas ubicaciones/IP
async function detectAndNotifyNewLocationLogin(adminEmail, adminName) {
    try {
        const currentIP = await getClientIP();
        if (!currentIP || currentIP === 'unknown') {
            return; // No podemos detectar si no tenemos IP
        }

        // Obtener IPs conocidas del localStorage
        const knownIPsKey = `knownIPs:${adminEmail}`;
        const knownIPs = JSON.parse(localStorage.getItem(knownIPsKey) || '[]');
        
        // Verificar si la IP es nueva
        const isNewIP = !knownIPs.includes(currentIP);
        
        if (isNewIP) {
            // Agregar la nueva IP a la lista de conocidas
            knownIPs.push(currentIP);
            // Mantener solo las últimas 10 IPs
            if (knownIPs.length > 10) {
                knownIPs.shift();
            }
            localStorage.setItem(knownIPsKey, JSON.stringify(knownIPs));
            ...
            await sendSecurityNotification('login_new_location', {
                ...
                adminName: adminName,
                ipAddress: currentIP,
                location: 'Desconocida', // Se podría mejorar con un servicio de geolocalización
                timestamp: new Date().toLocaleString('es-ES')
            });
            
            console.log(`⚠️ Login desde nueva IP detectado: ${currentIP} para ${adminEmail}`);
        }
    } catch (error) {
        console.warn('Error al detectar nueva ubicación de login:', error);
    }
}

// Función para enviar notificaciones de seguridad por email
async function sendSecurityNotification(type, data) {
    try {
        const adminEmails = ['aytocobreros@gmail.com'];
        if (SUPER_ADMIN && SUPER_ADMIN.email) {
            adminEmails.push(SUPER_ADMIN.email);
        }
        ...
        switch (type) {
            case 'admin_created':
                subject = `🔐 Nueva Cuenta de Administrador Creada - Ayuntamiento de Cobreros`;
                emailBody = `
                    <h2>Nueva Cuenta de Administrador Creada</h2>
                    <p><strong>Se ha creado una nueva cuenta de administrador en el sistema:</strong></p>
                    <ul>
                        <li><strong>Nombre:</strong> ${data.adminName}</li>
                        <li><strong>Email:</strong> ${data.adminEmail}</li>
                        <li><strong>Creado por:</strong> ${data.createdBy} (${data.createdByEmail})</li>
                        <li><strong>Fecha y hora:</strong> ${data.timestamp}</li>
                    </ul>
                    <p style="color: #d32f2f; font-weight: bold;">⚠️ Si no reconoces esta acción, contacta inmediatamente con el administrador del sistema.</p>
                `;
                break;
            case 'admin_deleted':
                subject = `🔐 Cuenta de Administrador Eliminada - Ayuntamiento de Cobreros`;
                emailBody = `
                    <h2>Cuenta de Administrador Eliminada</h2>
                    <p><strong>Se ha eliminado una cuenta de administrador del sistema:</strong></p>
                    <ul>
                        <li><strong>Nombre:</strong> ${data.adminName}</li>
                        <li><strong>Email:</strong> ${data.adminEmail}</li>
                        <li><strong>Eliminado por:</strong> ${data.deletedBy} (${data.deletedByEmail})</li>
                        <li><strong>Fecha y hora:</strong> ${data.timestamp}</li>
                    </ul>
                    <p style="color: #d32f2f; font-weight: bold;">⚠️ Si no reconoces esta acción, contacta inmediatamente con el administrador del sistema.</p>
                `;
            ...
            case 'login_new_location':
                subject = `🔐 Inicio de Sesión desde Nueva Ubicación - Ayuntamiento de Cobreros`;
                emailBody = `
                    <h2>Inicio de Sesión desde Nueva Ubicación</h2>
                    <p><strong>Se ha detectado un inicio de sesión desde una ubicación o IP no reconocida:</strong></p>
                    <ul>
                        <li><strong>Administrador:</strong> ${data.adminName} (${data.adminEmail})</li>
                        <li><strong>IP:</strong> ${data.ipAddress}</li>
                        <li><strong>Ubicación:</strong> ${data.location || 'Desconocida'}</li>
                        <li><strong>Fecha y hora:</strong> ${data.timestamp}</li>
                    </ul>
                    <p style="color: #d32f2f; font-weight: bold;">⚠️ Si no fuiste tú, cambia tu contraseña inmediatamente y contacta con el administrador del sistema.</p>
                `;
                break;
            default:
                return false;
        ...
        const response = await fetch(`${CLOUD_FUNCTIONS_BASE_URL}/sendEmail`, {
            ...
            body: JSON.stringify({
                ...
                data: {
                    title: subject,
                    content: emailBody,
                    timestamp: data.timestamp || new Date().toLocaleString('es-ES')
                }
            })
        });
        ...
        const result = await response.json();
        if (result.success) {
            console.log(`✅ Notificación de seguridad enviada: ${type}`, result.messageId);
            return true;
        } else {
            console.error('❌ Error al enviar notificación de seguridad:', result.error);
            return false;
        }
    } catch (error) {
        console.error('❌ Error al enviar notificación de seguridad:', error);
        return false;
    }
}
...
async function register() {
    ...
    // Buscar notificationConsent en el formulario de registro (puede tener diferentes IDs)
    const notificationConsentEl = document.getElementById('notificationConsentRegister') || document.getElementById('notificationConsent') || document.getElementById('notificationConsentPush');
    const notificationConsent = notificationConsentEl ? notificationConsentEl.checked : false;
...
async function deleteAdmin(adminId) {
    ...
    // Logs de auditoría
    if (typeof window !== 'undefined' && window.auditLogSystem) {
        await window.auditLogSystem.log('ADMIN_DELETED', {
            adminEmail: admin.email,
            adminName: admin.name,
            adminId: admin.id,
            deletedBy: currentUser?.email || 'system',
            deletedByEmail: currentUser?.email || 'system',
            ipAddress: await getClientIP() || 'unknown'
        }, currentUser?.email || 'system', currentUser?.email || 'system');
    }
    ...
    // Enviar notificación de seguridad por email
    try {
        await sendSecurityNotification('admin_deleted', {
            adminEmail: admin.email,
            adminName: admin.name,
...
async function enviarNotificacionDesdeFormulario() {
    ...
    if (submitBtn) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Enviando aviso...';
    }
    ...
    try {
        ...
        if (result?.notification) {
            // Mensaje de confirmación más descriptivo
            const pushCount = result.push?.success ? '✓ Push enviado' : '';
            const emailCount = result.email?.success > 0 ? `✓ Email enviado a ${result.email.success} usuario(s)` : '';
            const recipientsInfo = alcance === 'localities' && localidades.length > 0 
                ? ` en ${localidades.length} localidad(es)` 
                : ' a todos los usuarios';
            
            let successMessage = `✅ Aviso "${titulo}" enviado correctamente${recipientsInfo}.`;
            if (pushCount) successMessage += ` ${pushCount}.`;
            if (emailCount) successMessage += ` ${emailCount}.`;
        ...
        showNotification('❌ No se pudo enviar la notificación. Por favor, verifique su conexión e inténtelo de nuevo.', 'error');
    } finally {
        if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.classList.remove('loading');
            submitBtn.innerHTML = '<i class="fas fa-paper-plane"></i> Enviar Aviso';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof AccessibilityModule !== 'undefined' && AccessibilityModule.init) {
        setTimeout(() => {
            AccessibilityModule.init();
        }, 500);
    }
});
// Fin del archivo script.js - Versión 3.4 - Cache bust v6